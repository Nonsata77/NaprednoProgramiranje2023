...
#include <ctime>
using namespace std;
//При секое извршување на кодот во компаилерот ги печати секогаш различно (рандом) каде што исто така се внесени плус 2 џокери.
...
constexpr size_t PAQUET_MAX = 54; // 13 * 4 + 2 jokeri
...
Carte* createStdPaquet(Carte paquet[], std::size_t size)
{
    ...
    // Mnozenje x 4
    for(int i=1; i<4; ++i) { copy(paquet, paquet+cards, paquet+cardsi); }

    for(int i=0; i<4; ++i) {
        for(size_t j=0; j<cards; ++j) { paquet[j+cardsi].boja = boi[i]; }
    }
    // Vnesuvanje na jokeri
